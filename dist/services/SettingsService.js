...
class SettingsService {
    static async getSetting(key) {
        const result = await (0, database_1.getDb)().select().from(schema_1.settings).where((0, drizzle_orm_1.eq)(schema_1.settings.key, key)).limit(1);
    ...
    static async setSetting(key, value, incomingVersion) {
        ...
        if (existing) {
            ...
            const updated = await (0, database_1.getDb)().update(schema_1.settings)
            ...
            const inserted = await (0, database_1.getDb)().insert(schema_1.settings)
                .values({
