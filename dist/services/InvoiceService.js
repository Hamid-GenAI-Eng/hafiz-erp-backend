...
class InvoiceService {
    static async getNextInvoiceNumber() {
        const result = await database_1.db.select({ invoice_number: schema_1.invoices.invoice_number })
    ...
    static async getAllInvoices() {
        return await database_1.db.select().from(schema_1.invoices).where((0, drizzle_orm_1.sql) `${schema_1.invoices.deleted_at} IS NULL`).orderBy((0, drizzle_orm_1.sql) `${schema_1.invoices.date} DESC, ${schema_1.invoices.time} DESC`);
    }
    static async getInvoiceById(id) {
        const invoiceResult = await database_1.db.select().from(schema_1.invoices).where((0, drizzle_orm_1.eq)(schema_1.invoices.id, id)).limit(1);
        if (!invoiceResult || invoiceResult.length === 0)
            return null;
        const items = await database_1.db.select().from(schema_1.invoice_items).where((0, drizzle_orm_1.eq)(schema_1.invoice_items.invoice_id, id));
        return {
            ...invoiceResult[0],
            items
        };
    ...
    static async createInvoice(data) {
        ...
        if (data.items && data.items.length > 0) {
            for (const item of data.items) {
                ...
                // Deduct stock with concurrency guard
                if (item.product_id && item.quantity > 0) {
                    ...
                    if (updateResult.changes === 0) {
                        // Revert changes or throw error. For safety, we throw here. 
        ...
        // 4. Logistics Income
        if (data.loaders && data.loaders.length > 0 && newInvoice.status !== 'Draft') {
    ...
    static async reverseInvoiceEffects(invoice) {
        ...
        // 3. Reverse Logistics Income
        if (invoice.status !== 'Draft') {
            const logisticsIncomes = await database_1.db.select().from(schema_1.logistics_expenses).where((0, drizzle_orm_1.eq)(schema_1.logistics_expenses.invoice_id, invoiceId));
    ...
    static async deleteInvoice(id) {
        ...
        // Reverse effects
        await this.reverseInvoiceEffects(invoice);
        // Soft delete invoice
        await database_1.db.update(schema_1.invoices).set({
            status: 'cancelled',
            deleted_at: new Date(),
        ...
        // Soft delete items
        await database_1.db.update(schema_1.invoice_items).set({
            deleted_at: new Date(),
            updated_at: new Date()
    ...
    static async updateInvoice(id, data) {
        ...
        // Physically delete old invoice items to insert new ones
        ...
        if (updatedInvoice.customer_id && updatedInvoice.status !== 'Draft') {
            if (updatedInvoice.grand_total > 0 || updatedInvoice.amount_paid > 0) {
                await CrmService_1.CrmService.createLedgerEntry({
                    id: (0, crypto_1.randomUUID)(),
        ...
        // 4. Logistics Income
        if (data.loaders && data.loaders.length > 0 && updatedInvoice.status !== 'Draft') {
            for (const loader of data.loaders) {
                if (loader.vehicle_id && loader.fee > 0) {
                    await database_1.db.insert(schema_1.logistics_expenses).values({
                        id: (0, crypto_1.randomUUID)(),
