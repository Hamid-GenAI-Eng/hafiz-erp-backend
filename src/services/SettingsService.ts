...
export class SettingsService {
  static async getSetting(key: string) {
    const result = await getDb().select().from(settings).where(eq(settings.key, key)).limit(1);
  ...
  static async setSetting(key: string, value: string, incomingVersion?: number) {
    ...
    if (existing) {
      ...
      const updated = await getDb().update(settings)
        .set({ value, version: existing.version + 1, updated_at: new Date() })
        .where(eq(settings.id, existing.id))
        .returning();
      return updated[0];
    } else {
      const inserted = await getDb().insert(settings)
        .values({
          id: randomUUID(),
          key,
          value,
