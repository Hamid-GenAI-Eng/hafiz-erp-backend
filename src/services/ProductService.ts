...
export class ProductService {
  ...
  static async createProduct(data: any) {
    ...
    if (newProduct.supplier_id && newProduct.current_qty > 0) {
      ...
      return db.transaction((tx: any) => {
        ...
        // 2. Get Supplier
        const suppArray = tx.select().from(suppliers).where(eq(suppliers.id, newProduct.supplier_id)).limit(1).all();
  ...
  static updateProduct(id: string, data: any, incomingVersion: number) {
    return db.transaction((tx: any) => {
      ...
      tx.update(products).set(updatedData).where(eq(products.id, id)).run();
      ...
      if (qty_difference !== 0 && existing.supplier_id) {
        ...
        if (billAmount > 0 || paymentAmount > 0) {
          tx.insert(supplier_ledgers).values({
            id: randomUUID(),
