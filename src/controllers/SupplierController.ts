...
export class SupplierController {
  ...
  static async createSupplier(req: Request, res: Response) {
    try {
      ...
      let supplier = await SupplierService.createSupplier(data);

      // Create an opening balance ledger entry if requested
      if (openingBalance !== 0) {
        const isDebt = openingBalance > 0;
        await SupplierService.createLedgerEntry({
          id: randomUUID(),
          supplier_id: newId,
        ...
        const updatedSupplier = await SupplierService.getSupplierById(newId);
        if (updatedSupplier) supplier = updatedSupplier as any;
      }
